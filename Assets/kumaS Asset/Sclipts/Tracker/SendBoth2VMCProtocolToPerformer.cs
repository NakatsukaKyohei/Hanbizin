...
{
    public class SendBoth2VMCProtocolToPerformer : SendDataBase
    {
        ...
        public override void Send()
        {
            if (body && cli)
            {
                ...
                for (int i = 0; i < 10; i++)
                {
                    SendTrackerInfo(i, position[i] + offset + offset_each[i], rotation[i]);
                }
        ...
        private void SendTrackerInfo(int idx, Vector3 pos, Quaternion rot)
        {
            cli.Send("VMC/Ext/Tra/Pos", "kumaS_VirtualTracker_" + idx, pos.x, pos.y, pos.z, rot.x, rot.y, rot.z, rot.w);
        }
